...
module.exports = app => {
  ...
  app.get(
    apiPrefix + "/users",
    expressJwt({ secret: JWT_SECRET, credentialsRequired: false }),
    app.controllers.user.find
  );
  app.get(
    apiPrefix + "/users/:id",
    expressJwt({ secret: JWT_SECRET }),
    app.controllers.user.findById
  );
  app.patch(
    apiPrefix + "/users",
    expressJwt({ secret: JWT_SECRET }),
  ...
  app.delete(
    apiPrefix + "/users",
    expressJwt({ secret: JWT_SECRET }),
    app.controllers.user.destroy
  );

  // Auth Routes
  app.post(apiPrefix + "/auth", app.controllers.auth.login);
  app.post(
    apiPrefix + "/auth/password/reset/start",
    app.controllers.auth.startPasswordReset
  );
  app.get("/password/reset/:token", app.controllers.auth.renderForm);
  app.post(
    apiPrefix + "/auth/password/reset/",
    app.controllers.auth.savePasswordReset
  );
